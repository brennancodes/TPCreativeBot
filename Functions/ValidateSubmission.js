                ...
                if (x.message.content != null){
                    ...
                    if (x.message.content.includes(interaction.user.id) && x.message.content.includes("map submission [")){
                        ...
                        if (config.mtcSettings.preventResubmit){
                            if (x.message.content.substring(x.message.content.indexOf(`\``)+1, x.message.content.lastIndexOf(`\``)) == mapId){
                                isValid = false;
                                interaction.editReply({content:`This map has already been submitted, most recently on ${x.message.createdTimestamp.toLocaleString()}.`, flags:MessageFlags.Ephemeral})
                                return isValid;
                            }
                        }
                        // If we're slowing submissions, block submission if it's been less than the cooldown
                            ...
                            if (((currentDate - x.message.createdTimestamp)/3600000).toFixed(2) < config.mtcSettings.submitCooldownHours){
                                isValid = false;
                                interaction.editReply({content:`You are submitting too frequently. Try again in ${((config.mtcSettings.submitCooldownHours*60) - ((currentDate - x.message.createdTimestamp)/60000)).toFixed(2)} minutes.`,flags:MessageFlags.Ephemeral})
                                return isValid;
                            }
                        }
                    }
