...
module.exports = (client) => {
    client.on("interactionCreate", async interaction => {
        try {
            ...
            const url = `https://tagpro-secret.koalabeast.com/mtc/rest/updatemap/${mapId}?category=${playlist.toLowerCase()}&weight=${weight}`
            axios({method:'post',url:url,headers:headers}).then(function(resp){
                if (resp.data && resp.data.includes("Updated map")){
                    ...
                    searchMaps().then(function(x){
                        const imageUrl = `https://static.koalabeast.com/images/maps/${x.key.split(" ").join("_").trim()}-small.png`
                        ...
                        const embed = new EmbedBuilder()
                            ...
                            .setTimestamp();
                        const mtcAdminChannel = client.channels.cache.get(config.channels.mtcAdmin);
