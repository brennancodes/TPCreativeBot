...
module.exports.execute = async (interaction) => {
    try {
        ...
        if (interaction.commandName != "throwbacksummary"){
            return false;
        }
        ...
        const embed = new EmbedBuilder()
                .setColor("#186360")
                .setAuthor({name:"State of Throwback Rotation",iconURL:"https://imgur.com/QWrriCS.png"})
                .setTimestamp()

        for (var i = 0; i < maps.length; i++){
            if (maps[i].weight === 0.1){
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('ShareToChannel---thsummary').setStyle(ButtonStyle.Danger).setLabel('Share 📢'),
