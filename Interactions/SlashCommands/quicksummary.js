...
module.exports.execute = async (interaction) => {
    try {
        ...
        if (!interaction.commandName.includes("quicksummary")){
            return false;
        }
        ...
        const embed = new EmbedBuilder()
                .setColor("#186360")
                .setAuthor({name:"State of Casual Rotation",iconURL:"https://imgur.com/QWrriCS.png"})
                .setTimestamp()

        for (var i = 0; i < maps.length; i++){
            // TO DO: FIND A WAY TO EFFICIENTLY CONDITIONALLY PULL DATA BASED ON SUMMARY TYPE
            if (interaction.commandName)
            if (maps[i].category === 'Rotation' && maps[i].inCasualRotation === true){
        ...
        // TO DO: FIND A WAY TO EFFICIENTLY CONDITIONALLY APPLY CUSTOM ID
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('ShareToChannel---csummary').setStyle(ButtonStyle.Danger).setLabel('Share 📢'),
